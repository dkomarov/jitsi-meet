...
export default class ScreenshotCaptureSummary {
    ...
    constructor(state: IReduxState) {
        ...
        this._streamWorker = new Worker(workerUrl, {
    ...
    async start(jitsiTrack: any) {
        if (!window.OffscreenCanvas) {
            logger.warn(
                "Can't start screenshot capture, OffscreenCanvas is not available"
            );
    ...
    _handleWorkerAction(message: { data: { id: number; imageBlob?: Blob } }) {
        ...
        if (
            id === TIMEOUT_TICK &&
            imageBlob &&
            this._queue.length < SCREENSHOT_QUEUE_LIMIT
        ) {
